  ...
  $(function () {
   ...
   (function bindDeleteButton() {
      ...
      deleteBtn.addEventListener("click", async (e) => {
        ...
        if (!confirm(`${targetDate} Do you want to delete the diary?`)) return;

        try {
          const res = await fetch(`/api/diary/${targetDate}`, {
            method: "DELETE",
          });
          const data = await res.json();
          ...
          const $cell = $(`.c-cal__cel[data-day='${targetDate}']`);
          if ($cell.length) {
            $cell.removeAttr("data-name data-notes");
            $cell.removeClass(
              "event event--important event--birthday event--festivity"
            );
          }

          renderEmoji(targetDate, "");
